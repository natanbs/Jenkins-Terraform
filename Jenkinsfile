def tfCmd(String command, String options = '') {
	...
	sh ( script: "echo ${command} ${options} && cd $WORKSPACE/main && export AWS_PROFILE=${PROFILE} && export TF_ENV_profile=${PROFILE} && terraform init && terraform ${command} ${options}", returnStatus: true)
}
